...
class MultiWindow:
    ...
    def __init__(self, width, height):
        ...
        self.water_plot = self.axs[0, 0].imshow(self.h[0,0].clone().detach().cpu().numpy(), cmap="Blues", vmin=0, vmax=0.1, alpha=self.img_alpha)
        self.momentum_u_plot = self.axs[0, 1].imshow(self.u[0,0].clone().detach().cpu().numpy(), cmap="bwr", vmin=-0.5, vmax=0.5, alpha=self.img_alpha)
        self.momentum_v_plot = self.axs[0, 2].imshow(self.v[0,0].clone().detach().cpu().numpy(), cmap="bwr", vmin=-0.5, vmax=0.5, alpha=self.img_alpha)
