...
def get_Net(params, spline_variables):
    ...
    elif params.net == "StackedSaltmarshUNet":
        net = StackedSaltmarshUNet(spline_variables, hidden_size=params.hidden_size)
	return net
...
class StackedSaltmarshUNet(nn.Module):
	# inspired by UNet taken from: https://github.com/milesial/Pytorch-UNet/blob/master/unet/unet_model.py
	...
	def __init__(self, spline_variables, hidden_size=64, interpolation_size=8, bilinear=True, input_size=4, residuals=False):
		...
		:interpolation_size: size of first interpolation layer for v_cond and v_mask
		"""
		super(StackedSaltmarshUNet, self).__init__()
		self.hidden_size = hidden_size
		self.bilinear = bilinear
		self.input_size = input_size
        ...
        # Hidden size for SWE part of this UNet
        self.hidden_state_size_SWE = spline_variables["h"].hidden_size() + spline_variables["u"].hidden_size() + spline_variables["v"].hidden_size()

        # Hidden size for biomorphological processes part of this UNet
        self.hidden_state_size_SB = spline_variables["S"].hidden_size() + spline_variables["B"].hidden_size()

		self.residuals = residuals
		
        # Interpolate masks and conditions
		self.interpol = nn.Conv2d(input_size,interpolation_size,kernel_size=2) # interpolate v_cond (2) and v_mask (1) from 4 surrounding fields

        # We do the same in-conv for both UNets
		self.inc = DoubleConv(self.hidden_state_size+interpolation_size, hidden_size) # input: hidden_state + interpolation of v_cond and v_mask

        # The rest of the UNet is separated for both timescales
		self.swe_down1 = Down(hidden_size, 2*hidden_size)
		self.swe_down2 = Down(2*hidden_size, 4*hidden_size)
		self.swe_down3 = Down(4*hidden_size, 8*hidden_size)
		factor = 2 if bilinear else 1
		self.swe_down4 = Down(8*hidden_size, 16*hidden_size // factor)
		self.swe_up1 = Up(16*hidden_size, 8*hidden_size // factor, bilinear)
		self.swe_up2 = Up(8*hidden_size, 4*hidden_size // factor, bilinear)
		self.swe_up3 = Up(4*hidden_size, 2*hidden_size // factor, bilinear)
		self.swe_up4 = Up(2*hidden_size, hidden_size, bilinear)
		self.swe_outc = OutConv(hidden_size, self.hidden_state_size_SWE)

        # ... and for sediment+vegetation
		self.sb_down1 = Down(hidden_size, 2*hidden_size)
		self.sb_down2 = Down(2*hidden_size, 4*hidden_size)
		self.sb_down3 = Down(4*hidden_size, 8*hidden_size)
		factor = 2 if bilinear else 1
		self.sb_down4 = Down(8*hidden_size, 16*hidden_size // factor)
		self.sb_up1 = Up(16*hidden_size, 8*hidden_size // factor, bilinear)
		self.sb_up2 = Up(8*hidden_size, 4*hidden_size // factor, bilinear)
		self.sb_up3 = Up(4*hidden_size, 2*hidden_size // factor, bilinear)
		self.sb_up4 = Up(2*hidden_size, hidden_size, bilinear)
		self.sb_outc = OutConv(hidden_size, self.hidden_state_size_SB)
	...
	def to(self, torch_device):
		super(StackedSaltmarshUNet, self).to(torch_device)
		self.output_scalar = self.output_scalar.to(torch_device)
		return self
	...
	def forward(self, hidden_state, uv_cond, uv_mask, S_cond, S_mask):
		...
		x2 = self.swe_down1(x1)
		x3 = self.swe_down2(x2)
		x4 = self.swe_down3(x3)
		x5 = self.swe_down4(x4)
		x = self.swe_up1(x5, x4)
		x = self.swe_up2(x, x3)
		x = self.swe_up3(x, x2)
		x = self.swe_up4(x, x1)
		x = self.swe_outc(x)
        ...
        out[:, :self.hidden_state_size_SWE, :, :] = out_swe
        out[:, self.hidden_state_size_SWE:, :, :] = out_sb
