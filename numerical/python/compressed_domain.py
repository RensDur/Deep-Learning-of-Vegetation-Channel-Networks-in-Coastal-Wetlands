...
class CompressedDomain:
    ...
    def __init__(self, width, height, meter_per_cell):
        ...
        # All quantities are represented in this one big tensor
        # 0. h
        # 1. S
        # 2. u
        # 3. v
        ...
        # 8. Ds * dS_dx
        # 9 .Ds * dS_dy
        self.data = torch.zeros(10, self.height, self.width)

        # Create identifiers for them (index-helpers)
        self.h = 0
        self.S = 1
        self.u = 2
        self.v = 3
    ...
    def init_preset_gaussian_wave(self, x_mu, y_mu, x_sig, y_sig, correlation=0):
        ...
        self.data[self.h, :, :] = 1 + A * torch.exp(
    ...
    def initialize(self):
        ...
        self.data[self.h, :, :] = self.H0

        # Vegetation density is zero everywhere, except for some randomly placed tussocks
        self.data[self.B, :, :] = 0
        self.data[self.B, torch.where(torch.rand(self.height, self.width) < self.pEst)] = self.k
    ...
    def get_h(self):
        return self.data[self.h, :, :].cpu().numpy()

    def get_S(self):
        return self.data[self.S, :, :].cpu().numpy()

    def get_B(self):
        return self.data[self.B, :, :].cpu().numpy()

    def get_u(self):
        return self.data[self.u, :, :].cpu().numpy()

    def get_v(self):
        return self.data[self.v, :, :].cpu().numpy()
